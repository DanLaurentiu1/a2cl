...
@pytest.fixture
def sample_topic_valid() -> Topic:
    return Topic(name="test", topic_type=TopicTypes.CONCEPTS)


@pytest.fixture
def sample_topic_invalid() -> Topic:
    return Topic(name="", topic_type=TopicTypes.CONCEPTS)


def test_topic_object_creation(sample_topic_valid: Topic):
    assert sample_topic_valid is not None


def test_topic_name_creation_valid(sample_topic_valid: Topic):
    assert sample_topic_valid.name == "test"


def test_topic_type_creation_valid(sample_topic_valid: Topic):
    assert sample_topic_valid.type == TopicTypes.CONCEPTS


def test_topic_to_json_valid(sample_topic_valid: Topic):
...
def test_topic_from_json_valid(sample_topic_valid: Topic):
    json_object = sample_topic_valid.to_json()
...
def test_topic_from_json_invalid(sample_topic_invalid: Topic):
