...
def test_plan_id_valid(sample_plan_valid):
    assert sample_plan_valid.id == 1

def test_plan_title_valid(sample_plan_valid):
    assert sample_plan_valid.title == "test"

def test_plan_title_setter_valid(sample_plan_valid):
    plan = sample_plan_valid
    plan.title = "newTitle"
    assert plan.title == "newTitle"

def test_plan_title_setter_invalid(sample_plan_valid):
    plan = sample_plan_valid
    with pytest.raises(ValueError) as e:
        plan.title = 123
    assert str(e.value) == "Title must be a non-empty string"

def test_plan_profile_valid(sample_plan_valid):
    assert sample_plan_valid.profile.name == "test"

def test_plan_problems_valid(sample_plan_valid):
