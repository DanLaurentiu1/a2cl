...
@pytest.fixture
def sample_problem_valid() -> Problem:
    return Problem(
        id=1,
        name="test",
...
@pytest.fixture
def sample_problem_invalid() -> Problem:
    return Problem(
        id=1,
        name="",
        topics=[Topic(name="test", topic_type=TopicTypes.CONCEPTS)],
...
def test_problem_object_creation(sample_problem_valid: Problem):
    assert sample_problem_valid is not None


def test_problem_name_valid(sample_problem_valid: Problem):
    assert sample_problem_valid.name == "test"


def test_problem_id_valid(sample_problem_valid: Problem):
    assert sample_problem_valid.id == 1


def test_problem_topics_valid(sample_problem_valid: Problem):
    assert isinstance(sample_problem_valid.topics[0], Topic)


def test_problem_difficulty_valid(sample_problem_valid: Problem):
    assert sample_problem_valid.difficulty == 1


def test_problem_acceptance_rate_valid(sample_problem_valid: Problem):
    assert sample_problem_valid.acceptanceRate == 55.1


def test_problem_to_json_valid(sample_problem_valid: Problem):
...
def test_problem_from_json_valid(sample_problem_valid: Problem):
    json_object = sample_problem_valid.to_json()
...
def test_problem_from_json_invalid(sample_problem_invalid: Problem):
