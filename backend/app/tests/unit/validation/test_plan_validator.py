...
@pytest.fixture
def plan_empty_title_not_string() -> Plan:
    return Plan(
        id=1,
        title=123,
        profile=Profile(name="test"),
...
@pytest.fixture
def plan_title_length_over_cap() -> Plan:
    return Plan(
        id=1,
...
@pytest.fixture
def plan_title_with_special_characters() -> Plan:
    return Plan(
        id=1,
        title="test!!",
        profile=Profile(name="test"),
...
def test_validate_plan_valid(plan_valid: Plan):
    assert plan_validator.validate_plan(plan_valid) is None


def test_validate_plan_invalid_title(plan_empty_title_not_string: Plan):
    with pytest.raises(ValueError) as e:
        plan_validator.validate_plan(plan_empty_title_not_string)
    assert str(e.value) == "Title must be a string"


def test_validate_plan_invalid_title_over_cap(plan_title_length_over_cap: Plan):
    with pytest.raises(ValueError) as e:
        plan_validator.validate_plan(plan_title_length_over_cap)
    assert str(e.value) == "Plan title must be <= 100 characters"


def test_validate_plan_title_has_special_characters(
    plan_title_with_special_characters: Plan,
):
    with pytest.raises(ValueError) as e:
